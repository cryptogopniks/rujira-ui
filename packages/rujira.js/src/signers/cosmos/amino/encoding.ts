...
function decodeMultisigPubkey(data: Uint8Array): MultisigThresholdPubkey {
  ...
  const prefixFromReader = reader.splice(
    0,
    pubkeyAminoPrefixMultisigThreshold.length
  );
  if (
  ...
  if (reader.shift() != 0x08) {
    throw new Error(
      "Invalid multisig data. Expecting 0x08 prefix before threshold."
    );
  }
  ...
  while (reader.length > 0) {
    ...
    if (reader.shift() != 0x12) {
      throw new Error(
        "Invalid multisig data. Expecting 0x12 prefix before participant pubkey length."
      );
    }
