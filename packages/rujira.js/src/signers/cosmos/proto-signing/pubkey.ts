...
import { PubKey as CosmosCryptoEd25519Pubkey } from "../types/cosmos/crypto/ed25519/keys";
import { LegacyAminoPubKey } from "../types/cosmos/crypto/multisig/keys";
  ...
  if (isSecp256k1Pubkey(pubkey)) {
    ...
    return Any.fromPartial({
      ...
      value: Uint8Array.from(
        CosmosCryptoSecp256k1Pubkey.encode(pubkeyProto).finish()
      ),
    });
    ...
    return Any.fromPartial({
      ...
      value: Uint8Array.from(
        CosmosCryptoEd25519Pubkey.encode(pubkeyProto).finish()
      ),
    });
...
export function decodeOptionalPubkey(
  ...
  if (pubkey.typeUrl) {
    if (pubkey.value.length) {
      ...
      throw new Error(
        `Pubkey is an Any with type URL '${pubkey.typeUrl}' but an empty value`
      );
    }
  } else {
    if (pubkey.value.length) {
      throw new Error(
        `Pubkey is an Any with an empty type URL but a value set`
      );
    } else {
