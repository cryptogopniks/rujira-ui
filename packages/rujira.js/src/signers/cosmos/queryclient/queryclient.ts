...
import { ProofOps } from "../types/tendermint/crypto/proof";
...
export class QueryClient {
  ...
  public static withExtensions(cometClient: Comet38Client): QueryClient;
  ...
  public static withExtensions(
    ...
    const extensions = extensionSetups.map((setupExtension) =>
      setupExtension(client)
    );
  ...
  private readonly cometClient: Comet38Client;

  public constructor(cometClient: Comet38Client) {
  ...
  public async queryStoreVerified(
    ...
    const { height, proof, key, value } = await this.queryRawProof(
      store,
      queryKey,
      desiredHeight
    );
  ...
  public async queryRawProof(
    ...
    const { key, value, height, proof, code, log } =
    ...
    if (!proof || proof.ops.length !== 2) {
      throw new Error(
        `Expected 2 proof ops, got ${proof?.ops.length ?? 0}. Are you using stargate?`
      );
    }
  ...
  private async getNextHeader(height?: number): Promise<Header> {
    ...
    while (!nextHeader) {
      ...
      const correctHeader = (
        await this.cometClient.blockchain(height, searchHeight)
      ).blockMetas
